 ...
 * @date 27 April 2015
 * @brief C source code for stack creation and management functions
 *
 */
#include <stdlib.h>
#include "stack.h"
...
int push(Stack *which_stack, void *ptr) {
  ...
  if ((which_stack->cells_used) >= (which_stack->max_cells)) { // fix to use >=
...
void* pop(Stack *which_stack) {
  ...
  if ((which_stack->cells_used) <= 0) { // make <=
